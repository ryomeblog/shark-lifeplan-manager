 ...
 * 資産作成・編集モーダル
 */
const AssetModal = observer(
  ({ visible, onDismiss, onSubmit, initialValues, year }) => {
    // 基本情報の状態
    ...
    useEffect(() => {
      if (initialValues) {
        ...
        const yearPerformance = initialValues.yearlyPerformance?.find(
          (p) => p.year === year,
        );
        if (yearPerformance) {
          setActualEndValue(
            yearPerformance.actualEndValue || yearPerformance.endValue,
          );
          setActualCapitalGains(
            yearPerformance.actualCapitalGains || yearPerformance.capitalGains,
          );
          setActualDividends(
            yearPerformance.actualDividends || yearPerformance.dividends,
          );
        }
      }
    }, [initialValues, year]);
          ...
          <ScrollView style={styles.content}>
            ...
            <List.Section title="キャピタルゲイン設定">
              ...
              >
                <List.Item
                  title="年次"
                  onPress={() =>
                    setCompoundingFrequency(ENUMS.compoundingFrequency.YEARLY)
                  }
                />
                <List.Item
                  title="月次"
                ...
                <List.Item
                  title="日次"
                  onPress={() =>
                    setCompoundingFrequency(ENUMS.compoundingFrequency.DAILY)
                  }
                />
              </List.Accordion>
            </List.Section>
            ...
            <List.Section title="インカムゲイン設定">
              ...
              >
                <List.Item
                  title="年次"
                  onPress={() => setPaymentFrequency("yearly")}
                />
                <List.Item
                  title="四半期"
              ...
              {actualDividends.map((dividend, index) => (
                <View key={index} style={styles.dividendRow}>
                  ...
                  <Button
                    ...
                    mode="text"
                    onPress={() => {
                      const updated = actualDividends.filter(
                        (_, i) => i !== index,
                      );
                      setActualDividends(updated);
                    }}
