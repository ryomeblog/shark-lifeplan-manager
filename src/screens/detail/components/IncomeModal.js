 ...
 * 収入作成・編集モーダル
 */
const IncomeModal = observer(({ visible, onDismiss, onSubmit, initialValues }) => {
  // フォームの状態管理
  ...
  return (
    <Portal>
      <Modal
        visible={visible}
        onDismiss={onDismiss}
        onShow={() => logModalShow('収入モーダル')}
        contentContainerStyle={styles.modalContainer}>
        <View style={styles.header}>
          <Text style={styles.title}>{initialValues ? '収入の編集' : '収入の追加'}</Text>
        </View>
          ...
          {formMode === 'single' && (
            <View style={styles.form}>
              ...
              <List.Accordion title="頻度" description={frequency} style={styles.input}>
                ...
                <List.Item title="四半期" onPress={() => setFrequency(ENUMS.frequency.QUARTERLY)} />
