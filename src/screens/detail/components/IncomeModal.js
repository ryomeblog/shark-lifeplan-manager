 ...
 * 収入作成・編集モーダル
 */
const IncomeModal = observer(
  ({ visible, onDismiss, onSubmit, initialValues }) => {
    // フォームの状態管理
    ...
    const groups = rootStore.groupStore.sortedIncomeGroups;
            ...
            {formMode === "single" && (
              <View style={styles.form}>
                ...
                >
                  <List.Item
                    title="1回のみ"
                    onPress={() => setFrequency(ENUMS.frequency.ONCE)}
                  />
                  <List.Item
                    title="毎月"
                    onPress={() => setFrequency(ENUMS.frequency.MONTHLY)}
                  />
                  <List.Item
                    title="四半期"
                    onPress={() => setFrequency(ENUMS.frequency.QUARTERLY)}
                  />
                  <List.Item
                    title="毎年"
                    onPress={() => setFrequency(ENUMS.frequency.YEARLY)}
                  />
                </List.Accordion>
                <List.Accordion
                  title="カテゴリ"
                ...
                {(frequency === ENUMS.frequency.ONCE ||
                  ...
                  frequency === ENUMS.frequency.YEARLY) && (
                  <DatePickerInput
                    label="発生日"
                    value={date}
                    onChange={setDate}
